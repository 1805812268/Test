...
public class VerticalScrollList extends FrameLayout {
	/**
	 * width of child
	 */
	...
	private int maxVisbleChild = 5;
	private int middleChildIndex = maxVisbleChild/2;
	private Context mContext;
	private Adapter mAdapter = null;
	public VerticalScrollList(Context context) {
		super(context);
		mContext = context;
		init();
	}
	public VerticalScrollList(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}
	public VerticalScrollList(Context context, AttributeSet attrs, int defStyle) {
	...
	public android.view.ViewGroup.LayoutParams getParams() {
		...
		int height = (childHeight +gap)*maxVisbleChild-gap;
		if(params == null){
			params =new android.view.ViewGroup.LayoutParams(android.view.ViewGroup.LayoutParams.WRAP_CONTENT,height);
		}else{
	...
	public void setAdapter(Adapter adapter){
		mAdapter = adapter;
		//currentChildIndex = middleChildIndex;
		currentChildIndex =0;
		removeAllViews();
		scrollTo(0, 0);
		if(!update()){
	...
	private boolean update(){
		...
		for(int i = 0;i<childNum;i++){
			//int viewPos = i%mAdapter.getCount();
			View child = mAdapter.getView(i, null, null);
			FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(childWidth,childHeight); 
			params.topMargin = i*(childHeight+gap);
	...
	/**
	 * get position of view we can see  
	 * @return
	 */
	...
	protected boolean scrollLast(){
		...
		//int showNum = getShowChildNumber();
	...
	private void scroll(int num){
		...
		mScroller.startScroll(startX, startY, 0,(childHeight+gap)*num, (int)(1000*animationTime/*Math.abs(num)*/));
		isMoving = true;
	...
	/**
	 * calculate visible child number
	 * @return
	 */
	public int calculateShowChildNumber(){
		int listHeight = getHeight();
		int itemNum = listHeight/(childHeight+gap);
		int childCount = getChildCount();
		
		if(itemNum >=childCount){
			return childCount;
		}else{
			View child =getChildAt(itemNum);
			int childTop = child.getTop();
			int leave = listHeight-childTop;
			int childHeight = child.getHeight();
			if(leave>=childHeight){
				return itemNum+1;
			}else{
				return itemNum;
			}
